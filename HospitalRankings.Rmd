...
#fit the data
fit <- knn3(Star_Rating ~ ., data = train_set, k=3)
f_hat <- predict(fit, newdata = test_set)
f_hat_max <- apply(f_hat,1,max)
pred <- apply(f_hat,1,which.max)
...
u = union(pred, truth)
t = table(factor(pred, u), factor(truth, u))
confusionMatrix(t)
```

Unfortunately, our accuracy decreased to 51.9%. This may be due to overfitting the data.
