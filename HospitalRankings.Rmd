...
Severity$Median.Charge <- sub('\\$','', as.character(levels(Severity$Mean.Charge))[Severity$Mean.Charge]) %>%
  as.numeric()
...
```{r}
#join all the datasets with facility ID (remove ealier years from hospitalinfection)
facilityID_data <- hospitalinfection %>%
  select(c(Facility.Id, Year, Infections.observed, Denominator))
%>%
  filter(Year > 2010, Facility.Id > 0)
