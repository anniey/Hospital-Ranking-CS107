...
We see that it appears like mortality rates decrease with higher number of cases for heart failure

```{r}
#let's look at the relationship for HF only
CH %>%
  filter(Measure.ID %in% c("MORT_30_HF", "MORT_30_AMI")) %>%
  ggplot(aes(Denominator, Score), colour = factor(Measure.ID)) +
  geom_point() +
  geom_smooth() 

```

Let's see what happens when we try to fit a regression
```{r}
library(broom)
res <- CH %>%
...
Looks like regression is a good way to predict mortality score
