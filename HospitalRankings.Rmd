...
o verify our findings for mortality, we will now check other databases that contain the same metric. In order to do that though, we must first find a way to join our national datasets, which uses provider ID, with our PCIbyhospital NY dataset, which uses facility ID. There are no common code between the two, so we will have to perform our join using hospital names.
...
#let's take a look at the correlation too
M <- cor(HCAHPS_sum)
corrplot(M, method = "circle", margins = c(1,1)) #plot matrix
```
...
#fit the data
fit <- knn3(Star_Rating ~., data = train_set, k=9)
...
From all of the analysis above, it looks like the best candidates for additional predictors of HCAHPS star rating is readmission, time to emergency room, and payments for pneumonia.
...
final_dat <- paymentandvalue %>%
  ...
  right_join(readmitHCAHPS, by = "Provider.ID") %>%
  select(-c(Number.of.Discharges, Number.of.Readmissions))
...
#changing the order of magnitude for payment, ER wait, readmission rate
final_dat <- final_dat %>%
  transform(Payment = Payment / 100, Score = Score / 10, readmissionRate = readmissionRate * 100)

#let's make the column names friendly
colnames(final_dat) <- c("ER_Wait", "Pay_PN", "Star_Rating", "Cleanliness", "NurseComm", "DoctorComm", "StaffResponsiveness", "PainMmgt.", "MedicationComm.", "Discharge_Info", "Care_Transition", "Overall_Rating", "Quietness", "Readmission Rate")
...
heatmap(as.matrix(final_dat_dist), margins = c(10,10))

```


Now, let's revist our knn model

```{r}
#let's find the best k
set.seed(1)
final_dat <- scale(final_dat)
