...
Our datasets are a mixture of national and NY databases. The former uses provider ID to tag hospitals while the later uses facility ID. There are no common code between the two, so we will have to perform our join using hospital names.
...
From the above, we see that the remaining metrics in HCAHPS has about a 83% accuracy rating in predicting star ratings.
...
#let's make the column names friendly
colnames(final_dat) <- c("ER_Wait", "Readmission", "Payment", "Star_Rating", "Cleanliness", "NurseComm", "DoctorComm", "StaffResponsiveness", "PainMmgt.", "MedicationComm.", "Discharge_Info", "Care_Transition", "Overall_Rating", "Quietness")
...
Unfortunately, it looks like none of the other metrics we choose has helped us improve our ratings.
